...
CliCommand CliParser::parse(const String& rawLine) const {
  ...
  CommandType poseCommand = COMMAND_INVALID;
  if (isSetFingerPoseCommand(tokens, tokenCount, poseCommand)) {
    return {poseCommand, 0, 0};
  }
...
bool CliParser::isSetFingerPoseCommand(const String tokens[], int tokenCount, CommandType& commandType) const {
  if (tokenCount != 2 || !tokens[0].equalsIgnoreCase("set")) {
    return false;
  }

  if (tokens[1].equalsIgnoreCase("one")) {
    commandType = COMMAND_SET_ONE;
    return true;
  }

  if (tokens[1].equalsIgnoreCase("two")) {
    commandType = COMMAND_SET_TWO;
    return true;
  }

  if (tokens[1].equalsIgnoreCase("three")) {
    commandType = COMMAND_SET_THREE;
    return true;
  }

  if (tokens[1].equalsIgnoreCase("four")) {
    commandType = COMMAND_SET_FOUR;
    return true;
  }

  if (tokens[1].equalsIgnoreCase("five")) {
    commandType = COMMAND_SET_FIVE;
    return true;
  }
