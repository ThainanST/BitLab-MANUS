...
AppController::AppController(const int* servoPins, const int* closeMaxAngles, const FingerMotionConfig& config)
  : hand(servoPins, closeMaxAngles, config), maxCloseAngles(closeMaxAngles), motionConfig(config) {}
...
void AppController::handleFingerPoseCommand(int openFingerId, const __FlashStringHelper* label) {
  if (hand.startPose(openFingerId)) {
    Serial.print(F("[SET] Pose started: "));
...
void AppController::handleGetStatusCommand() {
  ...
  Serial.print(F(" maxAngles="));
  for (int i = 0; i < 5; i++) {
    if (i > 0) {
      Serial.print(',');
    }
    Serial.print(hand.getMaxAngle(i + 1));
  }
...
void AppController::handleCliLine(const String& line) {
  ...
  if (command.type == COMMAND_SET_ONE) {
    handleFingerPoseCommand(1, F("one"));
    return;
  }

  if (command.type == COMMAND_SET_TWO) {
    handleFingerPoseCommand(2, F("two"));
    return;
  }

  if (command.type == COMMAND_SET_THREE) {
    handleFingerPoseCommand(3, F("three"));
    return;
  }

  if (command.type == COMMAND_SET_FOUR) {
    handleFingerPoseCommand(4, F("four"));
    return;
  }

  if (command.type == COMMAND_SET_FIVE) {
    handleFingerPoseCommand(5, F("five"));
    return;
  }
